...
function interaction(id; ONCHigh = true)
    ...
    try
        ...
        for p in pairs
            ...
            enames = findall("//interactionTypeGroup/interactionType/interactionPair/iterationConcept/minConceptItem/name", p)
            if !isempty(enames)
...
function interaction_within_list(idlist::Vector{String})
    ...
    try
        ...
        for p in pairs
            sev = nodecontent(findfirst("severity", p))
            desc = nodecontent(findfirst("description", p))
            enames = findall("//fullInteractionTypeGroup/fullInteractionType/interactionPair/iterationConcept/minConceptItem/name", p)
            if !isempty(enames)
                names = nodecontent.(enames)
                push!(interactions, (drug1=names[1], drug2=names[2], severity=sev, description=desc))
            end
